...
2. Assume common Indian spices/aromatics are available (don't filter for them):
...
3. Calculate match_score favoring recipes with more ingredient matches
4. Use LOWER() for case-insensitive matching
5. Order by match_score DESC
6. LIMIT 15 results
...
def _generate_basic_query(ingredients: List[str]) -> str:
    ...
    if not ingredients:
        return "SELECT rowid as recipe_id, recipe, ingredients, instruction FROM recipes LIMIT 10"
